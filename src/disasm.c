...
static int isXrelOpcode(uint8_t opcode) {
	switch (opcode) {
		case 0x01: // ora (zp,x)
		case 0x15: // ora zp,x
		case 0x16: // asl zp,x
...
int disasm_named_symbols(uint16_t pc, uint8_t *RAM, char *line, unsigned int max_line, int16_t bank, int32_t *eff_addr, char* type) {
	uint8_t opcode = debug_read6502(pc, bank);
    const char *numeric = mnemonics_c02[opcode];  // Always use c02 mnemonics for named symbol mode
	const char *mnemonic = sym_mnemonics[opcode];
	...
	*type = isControlFlow ? 0x10 : 0;

	int length = 1;
	strncpy(line, mnemonic, max_line);

	if (isZprel) {
        int addr_zp = debug_read6502(pc + 1, bank);
        struct line_symbol *symbol = sym_find_symbol_by_address(addr_zp);
        if (symbol == NULL) {
            snprintf(line, max_line, numeric, debug_read6502(pc + 1, bank), pc + 3 + (int8_t)debug_read6502(pc + 2, bank));
        } else {
            *type |= symbol->type;
		    snprintf(line, max_line, mnemonic, symbol->name, pc + 3 + (int8_t)debug_read6502(pc + 2, bank));
        }
		length = 3;
	} else {
		if (strstr(numeric, "%02x")) {
			length = 2;
			if (isBranch) {
                int addr = pc + 2 + (int8_t)debug_read6502(pc + 1, bank);
                struct line_symbol *symbol = sym_find_symbol_by_address(addr);
                if (symbol == NULL) {
                    snprintf(line, max_line, numeric, addr);
                } else {
                    *type |= symbol->type;
				    snprintf(line, max_line, mnemonic, symbol->name);
                }
			} else {
                int addr = debug_read6502(pc + 1, bank);
                struct line_symbol *symbol = sym_find_symbol_by_address(addr);
		...
		if (strstr(numeric, "%04x")) {
			length = 3;
            int addr = debug_read6502(pc + 1, bank) | (debug_read6502(pc + 2, bank) << 8);
            struct line_symbol *symbol = sym_find_symbol_by_address(addr);
            if (symbol == NULL) {
			    snprintf(line, max_line, numeric, addr);
            } else {
		...
		if (opcode == 0x00) {
			// BRK instruction is 2 bytes long
			length = 2;
		}
	}
