...
#define DBGKEY_SYMBOLS	SDLK_RCTRL                // Right control shows symbols.
...
void DEBUGRenderDisplay(int width, int height) {
    ...
    if (!showSymbols) {
        ...
        DEBUGRenderMinimalisticStack(register_lines);
    }
...
static void DEBUGRenderCode(int lines, int initialPC) {
	...
	for (int y = 0; y < lines; y++) { 							// Each line
        ...
        int size = showSymbols ?
            disasm_named_symbols(initialPC, RAM, buffer, sizeof(buffer), currentPCBank, &eff_addr, &type) :
		    disasm(initialPC, RAM, buffer, sizeof(buffer), currentPCBank, implied_status, &eff_addr);	// Disassemble code
		// Output assembly highlighting PC
        if ((type & 0x10) == 0) {
		    DEBUGString(dbgRenderer, DBG_ASMX+8, y, buffer, initialPC == regs.pc ? col_highlight : col_data);
        } else {
            DEBUGString(dbgRenderer, DBG_ASMX+8, y, buffer, initialPC == regs.pc ? col_highlight : col_data);
            // TODO: Highlight control flow instructions
            // DEBUGString2col(
            //     dbgRenderer,
            //     DBG_ASMX+8,
            //     y,
            //     buffer,
            //     initialPC == regs.pc ? col_highlight :
...
static int DEBUGRenderRegisters() {
	int n = 0,yc = 0;
	if (regs.is65c816) {
